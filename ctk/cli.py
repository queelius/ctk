...
def execute_ask_tool(
    ...
    """Thin shim over ``ctk.core.builtin_tools.execute_builtin_tool``.

    All builtin tool handlers now live in ``ctk.core.builtin_tools`` as
    self-dispatching ``BuiltinTool`` entries. This shim only adds the debug
    prints the TUI/CLI callers rely on and forwards to the registry.
    """
    if debug:
        import sys
