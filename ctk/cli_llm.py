    ...
    try:
        if provider_name == 'ollama':
            ...
            if provider.is_available():
                ...
                if args.model:
                    model_ids = [m.get('id', m.get('name', '')) if isinstance(m, dict) else str(m) for m in models]
                    if args.model in model_ids:
                        console.print(f"[green]OK[/green] - Model '{args.model}' is available")
                    else:
                        console.print(f"[yellow]Warning[/yellow] - Model '{args.model}' not found")
                return 0
            else:
            ...
            provider = OpenAIProvider({
                'api_key': api_key,
                'model': args.model or provider_config.get('default_model', 'gpt-3.5-turbo'),
            })
            ...
            provider = AnthropicProvider({
                'api_key': api_key,
                'model': args.model or provider_config.get('default_model', 'claude-3-haiku-20240307'),
            })
            ...
            console.print(f"[yellow]Warning[/yellow] - Testing not implemented for '{provider_name}'")
            return 1
...
def add_llm_commands(subparsers):
    ...
    # providers
    providers_parser = llm_subparsers.add_parser('providers', help='List configured providers')
    ...
    models_parser.add_argument('provider', nargs='?', default='ollama',
    ...
    test_parser.add_argument('provider', nargs='?', default='ollama',
                             help='Provider name (default: ollama)')
