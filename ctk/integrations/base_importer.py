...
class JSONBasedImporter(ImporterPlugin):
    ...
    def _parse_timestamp(
        self, timestamp: Any, formats: Optional[List[str]] = None
    ) -> Optional[datetime]:
        ...
        if isinstance(timestamp, str):
            ...
            for fmt in formats:
                try:
                    ...
                    logger.debug(
                        f"Failed to parse timestamp '{timestamp}' with format '{fmt}': {e}"
                    )
                    continue
    ...
    def _create_base_metadata(
        ...
        metadata = ConversationMetadata(
            ...
            or self._extract_created_time(conv_data)
            or datetime.now(),
            updated_at=overrides.get("updated_at")
            or self._extract_updated_time(conv_data)
            or datetime.now(),
            tags=overrides.get("tags", [self.name]),
            custom_data=overrides.get(
                "custom_data", self._extract_custom_metadata(conv_data)
            ),
        )
        return metadata

    def _safe_process_conversation(
        self, conv_data: Dict, index: int
        ...
        try:
            ...
            logger.error(
                f"Error processing conversation at index {index} in {self.name}: {e}"
            )
            return None
        ...
        try:
            ...
            for i, conv_data in enumerate(conversations_data):
                if not isinstance(conv_data, dict):
                    logger.warning(
                        f"Skipping non-dict conversation data at index {i} in {self.name}"
                    )
                    continue
            ...
            logger.info(
                f"Successfully imported {len(conversations)} conversations using {self.name}"
            )
