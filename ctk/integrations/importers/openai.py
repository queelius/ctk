...
class OpenAIImporter(ImporterPlugin):
    ...
    def _resolve_and_copy_image(self, file_service_url: str) -> Optional[str]:
        ...
        if not self.source_dir or not self.media_dir:
            logger.debug(
                f"source_dir or media_dir not provided, skipping image: {file_service_url}"
            )
            return None
        ...
        if file_service_url.startswith("sediment://"):
            logger.debug(
                f"Skipping sediment:// URL (files not in export): {file_service_url}"
            )
            return None
    ...
    def import_data(self, data: Any, **kwargs) -> List[ConversationTree]:
        ...
        for conv_data in data:
            ...
            for msg_id, msg_data in mapping.items():
                ...
                parent_id = (
                    None if (parent is None or parent in structural_nodes) else parent
                )
