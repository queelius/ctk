...
class GeminiImporter(ImporterPlugin):
    ...
    def validate(self, data: Any) -> bool:
        ...
        if isinstance(data, list) and data:
            sample = data[0]
            return (
                "model" in str(sample) and "gemini" in str(sample).lower()
            ) or "bard" in str(sample).lower()
    ...
    def import_data(self, data: Any, **kwargs) -> List[ConversationTree]:
        ...
        for conv_data in conv_list:
            conv_id = conv_data.get(
                "id", conv_data.get("conversation_id", str(uuid.uuid4()))
            )
                    ...
                    for part in parts:
                        if isinstance(part, str):
                            ...
                            if "text" in part:
                                ...
                                content.metadata["media"] = content.metadata.get(
                                    "media", []
                                )
