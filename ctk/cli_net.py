...
def add_net_commands(subparsers):
    ...
    net_subparsers = net_parser.add_subparsers(
        dest="net_command", help="Network operation to perform"
    )
    ...
    similar_parser = net_subparsers.add_parser(
        "similar", help="Find conversations similar to a given conversation"
    )
    similar_parser.add_argument(
        "id", nargs="?", help="Conversation ID (full or partial)"
    )
    similar_parser.add_argument("--db", "-d", required=True, help="Database path")
    similar_parser.add_argument(
    ...
    links_parser = net_subparsers.add_parser(
        "links", help="Build a graph of conversation relationships"
    )
    ...
    links_parser.add_argument(
        "--rebuild", action="store_true", help="Force rebuild even if graph exists"
    )

    # NETWORK command
    network_parser = net_subparsers.add_parser(
        "network", help="Display network statistics for current graph"
    )
    network_parser.add_argument("--db", "-d", required=True, help="Database path")
    network_parser.add_argument(
        "--rebuild", action="store_true", help="Recompute metrics even if cached"
    )

    # CLUSTERS command (community detection)
    clusters_parser = net_subparsers.add_parser(
        "clusters", help="Detect conversation communities/clusters"
    )
    clusters_parser.add_argument("--db", "-d", required=True, help="Database path")
    ...
    neighbors_parser = net_subparsers.add_parser(
        "neighbors", help="Show neighbors of a conversation in the graph"
    )
    neighbors_parser.add_argument("id", help="Conversation ID (full or partial)")
    neighbors_parser.add_argument("--db", "-d", required=True, help="Database path")
    neighbors_parser.add_argument(
        "--depth", type=int, default=1, help="Depth of neighbors to show (default: 1)"
    )

    # PATH command
    path_parser = net_subparsers.add_parser(
        "path", help="Find shortest path between two conversations"
    )
    path_parser.add_argument("source", help="Source conversation ID")
    path_parser.add_argument("target", help="Target conversation ID")
    path_parser.add_argument("--db", "-d", required=True, help="Database path")

    # CENTRAL command
    central_parser = net_subparsers.add_parser(
        "central", help="Find most central/connected conversations"
    )
    central_parser.add_argument("--db", "-d", required=True, help="Database path")
    ...
    outliers_parser = net_subparsers.add_parser(
        "outliers", help="Find least connected/isolated conversations"
    )
    outliers_parser.add_argument("--db", "-d", required=True, help="Database path")
    ...
    export_parser = net_subparsers.add_parser(
        "export", help="Export network graph to file (GraphML, DOT, GEXF)"
    )
...
def cmd_embeddings(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        if args.provider == "tfidf":
            ...
            console.print(
                f"[green]✓[/green] Fitted with {embedder.provider.vectorizer.max_features} features"
            )
    ...
    with ConversationDB(args.db) as db:
        ...
        if len(conv_id) < 36:
            ...
            if len(matches) == 1:
                ...
                console.print(
                    f"[yellow]Ambiguous prefix '{conv_id}', matches:[/yellow]"
                )
                for m in matches[:5]:
                ...
                console.print(
                    f"[red]No conversation found with prefix: {conv_id}[/red]"
                )
                return 1
        ...
        console.print(
            f"Finding conversations similar to: '{query_conv.title or '(untitled)'}'"
        )
        ...
        try:
            results = similarity.find_similar(
                query_conv, top_k=args.top_k, threshold=args.threshold, use_cache=True
            )
        except Exception as e:
            if "not fitted" in str(e).lower():
                console.print(
                    "[red]Error: TF-IDF not fitted. Run 'ctk net embeddings' first.[/red]"
                )
            else:
        ...
        for i, (conv, score) in enumerate(results, 1):
            ...
            table.add_row(str(i), f"{score:.3f}", conv_dict["id"][:8] + "...", title)
...
def cmd_links(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        if not session:
            console.print(
                "[red]Error: No embedding session found. Run 'ctk net embeddings' first.[/red]"
            )
            return 1
        ...
        if len(conversations) < 2:
            console.print(
                "[yellow]Need at least 2 conversations to build graph[/yellow]"
            )
            return 0
        ...
        console.print(
            f"[green]✓[/green] Built graph with {len(graph.nodes)} nodes and {len(graph.links)} edges"
        )
...
def cmd_network(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        if not graph_metadata:
            console.print(
                "[red]Error: No graph found. Run 'ctk net links' first to build a graph.[/red]"
            )
            return 1
...
def cmd_clusters(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        for i, community in enumerate(communities, 1):
            console.print(
                f"[bold]Community {i}[/bold] ({len(community)} conversations)"
            )
...
def cmd_neighbors(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        for nid, title, weight in neighbor_data[:20]:
            table.add_row(
                nid[:8] + "...", title[:45], f"{weight:.3f}" if weight > 0 else "-"
            )
    ...
    with ConversationDB(args.db) as db:
        ...
        if args.metric == "degree":
            ...
            try:
                ...
                console.print(
                    "[yellow]Eigenvector centrality failed to converge, using degree[/yellow]"
                )
                centrality = nx.degree_centrality(G)
        ...
        for i, (conv_id, score) in enumerate(sorted_nodes[: args.top_k], 1):
            conv = db.load_conversation(conv_id)
            title = conv.title if conv else "(untitled)"
            table.add_row(str(i), f"{score:.4f}", conv_id[:8] + "...", title[:45])
...
def cmd_outliers(args):
    ...
    with ConversationDB(args.db) as db:
        ...
        for i, (conv_id, score) in enumerate(sorted_nodes[: args.top_k], 1):
            ...
            table.add_row(str(i), str(degree), conv_id[:8] + "...", title[:45])
        ...
        try:
            if fmt == "graphml":
                ...
                try:
                    ...
                    console.print(
                        "[red]Error: pydot not installed. Install with: pip install pydot[/red]"
                    )
                    return 1
            elif fmt == "gml":
