...
Return ONLY valid JSON, nothing else."""
    ...
    def extract_text(self, conversation: ConversationTree) -> str:
        ...
        relevant_messages = (
            messages[:5] + messages[-5:] if len(messages) > 10 else messages
        )
                ...
                if isinstance(tags, list):
                    ...
                    for tag in tags:
                        if isinstance(tag, str):
                            ...
                            clean_tag = "".join(
                                c for c in clean_tag if c.isalnum() or c == "-"
                            )
    ...
    def batch_tag_conversations(
        self, conversations: List[ConversationTree], progress_callback=None
    ) -> Dict[str, List[str]]:
