...
class TFIDFTagger(BasePlugin):
    ...
    DOMAIN_PATTERNS = {
        ...
        "machine-learning": (
            r"\b(ml|machine learning|neural|network|training|model|dataset|tensor)\b"
        ),
    ...
    def tag_conversation(
        self, conversation: ConversationTree, num_tags: int = 10
    ) -> List[str]:
    ...
    def update_corpus_statistics(self, conversations: List[ConversationTree]):
        ...
        for conv in conversations:
            ...
            for word in words:
                self.document_frequencies[word] = (
                    self.document_frequencies.get(word, 0) + 1
                )
