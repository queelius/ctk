...
class ConversationModel(Base):
    ...
    paths: Mapped[List["PathModel"]] = relationship(
        "PathModel", back_populates="conversation", cascade="all, delete-orphan"
    )

    tags: Mapped[List["TagModel"]] = relationship(
        "TagModel", secondary=conversation_tags, back_populates="conversations"
    )

    embeddings: Mapped[List["EmbeddingModel"]] = relationship(
        "EmbeddingModel", back_populates="conversation", cascade="all, delete-orphan"
    )
...
class TagModel(Base):
    ...
    conversations: Mapped[List["ConversationModel"]] = relationship(
        "ConversationModel", secondary=conversation_tags, back_populates="tags"
    )
...
class PathModel(Base):
    ...
    )  # Is this the main path?
    length: Mapped[int] = mapped_column(Integer)  # Number of messages in path
    leaf_message_id: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # Last message in path

    # Relationships
    conversation: Mapped["ConversationModel"] = relationship(
        "ConversationModel", back_populates="paths"
    )
...
class EmbeddingModel(Base):
    ...
    aggregation_strategy: Mapped[str] = mapped_column(
        String
    )  # 'weighted_mean', 'mean', etc.
    aggregation_weights: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Role weights
    ...
    embedding_json: Mapped[list] = mapped_column(
        JSON
    )  # List of floats (numpy array serialized)
    ...
    # Relationships
    conversation: Mapped["ConversationModel"] = relationship(
        "ConversationModel", back_populates="embeddings"
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            name="uq_conversation_embedding",
        ),
    )
...
class SimilarityModel(Base):
    ...
    conversation1_id: Mapped[str] = mapped_column(
        String, ForeignKey("conversations.id")
    )
    conversation2_id: Mapped[str] = mapped_column(
        String, ForeignKey("conversations.id")
    )
