...
from ctk.core.models import ConversationTree, Message, MessageContent, MessageRole, ConversationMetadata
            ...
            with ConversationDB(db_path) as db:
                ...
                for i in range(3):
                    conv = ConversationTree(
                        id=f"conv-{i}",
                        title=f"Test {i}",
                        metadata=ConversationMetadata(source="test", model="test-model", tags=[f"tag{i}"])
                    )
