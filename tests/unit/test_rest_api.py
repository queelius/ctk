...
def sample_conversation():
    ...
    msg2 = Message(role=MessageRole.ASSISTANT, content=MessageContent(text="Hi there!"), parent_id=msg1.id)
...
class TestOrganizationEndpoints:
    ...
    def test_rename_conversation_success(self, rest_interface, mock_db):
        ...
        mock_db.update_conversation_metadata.assert_called_once_with("conv-123", title="New Title")
    ...
    def test_duplicate_conversation_success(self, rest_interface, mock_db):
        ...
        response = rest_interface.duplicate_conversation("conv-123", "Copy of Conversation")

        assert response.status == ResponseStatus.SUCCESS
        assert response.data["new_conversation_id"] == "new-conv-456"
        mock_db.duplicate_conversation.assert_called_once_with("conv-123", new_title="Copy of Conversation")
...
class TestTagEndpoints:
    ...
    def test_list_tags_success(self, rest_interface, mock_db):
        ...
        mock_db.get_all_tags = MagicMock(return_value=[
            {"name": "work", "count": 10},
...
class TestMetadataEndpoints:
    ...
    def test_list_models_success(self, rest_interface, mock_db):
        ...
        mock_db.get_models = MagicMock(return_value=[
    ...
    def test_list_sources_success(self, rest_interface, mock_db):
        ...
        mock_db.get_sources = MagicMock(return_value=[
    ...
    def test_get_timeline_success(self, rest_interface, mock_db):
        ...
        mock_db.get_conversation_timeline = MagicMock(return_value=[
        ...
        mock_db.get_conversation_timeline.assert_called_once_with(granularity="day", limit=30)


class TestTreePathEndpoints:
    """Tests for tree/path endpoints"""

    def test_get_conversation_tree_success(self, rest_interface, mock_db, sample_conversation):
    ...
    def test_list_conversation_paths_success(self, rest_interface, mock_db, sample_conversation):
    ...
    def test_get_conversation_path_success(self, rest_interface, mock_db, sample_conversation):
    ...
    def test_get_conversation_path_out_of_range(self, rest_interface, mock_db, sample_conversation):
...
class TestViewsEndpoints:
    ...
    def test_list_views_success(self, rest_interface):
        ...
        mock_view_store.list_views_detailed = MagicMock(return_value=[
            {"name": "favorites", "title": "My Favorites"},
            {"name": "work", "title": "Work Conversations"}
        ])

        with patch.object(rest_interface, '_get_view_store', return_value=mock_view_store):
    ...
    def test_get_view_json_success(self, rest_interface):
        ...
        mock_view_store = MagicMock()
        mock_view = MagicMock()
        mock_view.to_dict = MagicMock(return_value={
            "name": "test-view",
            "title": "Test View"
        })
    ...
    def test_evaluate_view_success(self, rest_interface, mock_db):
        ...
        mock_evaluated.items = [
            MagicMock(conversation_id="conv-1", title_override="Title 1", annotation="Note", path=None)
        ]
    ...
    def test_check_view_success(self, rest_interface, mock_db):
        ...
        mock_view_store = MagicMock()
        mock_view_store.check_view = MagicMock(return_value={
            "valid": True,
            "missing": [],
            "drifted": []
        })
...
class TestSearchEndpoints:
    ...
    def test_search_with_advanced_filters(self, rest_interface, mock_db):
        ...
        response = rest_interface.search_conversations("test", limit=50, options=options)
...
class TestInterfaceResponse:
    ...
    def test_error_response(self):
        ...
        response = InterfaceResponse.error(message="Something went wrong", errors=["Error 1"])
