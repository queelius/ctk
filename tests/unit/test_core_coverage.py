    ...
    def test_update_conversation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with ConversationDB(db_path) as db:
                ...
                msg = Message(
                    id="msg-1", role=MessageRole.USER, content=MessageContent(text="Hi")
                )
                conv.add_message(msg)
                db.save_conversation(conv)
...
class TestModelsCoverage:
    ...
    def test_message_with_parent(self):
        ...
        parent = Message(
            id="parent", role=MessageRole.USER, content=MessageContent(text="Question")
        )
    ...
    def test_metadata_with_tags(self):
        ...
        metadata = ConversationMetadata(
            source="test", model="test-model", tags=["tag1", "tag2", "tag3"]
        )
    ...
    def test_conversation_get_children(self):
        ...
        parent = Message(
            id="p1", role=MessageRole.USER, content=MessageContent(text="Q")
        )
        child1 = Message(
            id="c1",
            role=MessageRole.ASSISTANT,
            content=MessageContent(text="A1"),
            parent_id="p1",
        )
        child2 = Message(
            id="c2",
            role=MessageRole.ASSISTANT,
            content=MessageContent(text="A2"),
            parent_id="p1",
        )
    ...
    def test_message_roles(self):
        ...
        user_msg = Message(
            id="u", role=MessageRole.USER, content=MessageContent(text="Q")
        )
        assert user_msg.role == MessageRole.USER

        assistant_msg = Message(
            id="a", role=MessageRole.ASSISTANT, content=MessageContent(text="A")
        )
        assert assistant_msg.role == MessageRole.ASSISTANT

        system_msg = Message(
            id="s", role=MessageRole.SYSTEM, content=MessageContent(text="S")
        )
