...
class TestJSONLToolCallPreservation:
    ...
    def conversation_with_tool_calls(self):
        ...
        tool_call = ToolCall(
            ...
            status="completed",
            result={
                "results": [{"title": "Python Tutorial", "url": "https://example.com"}]
            },
        )
    ...
    def test_jsonl_preserves_tool_calls(self, conversation_with_tool_calls):
        ...
        ), "Content should be a list for structured messages"

        # Find tool call in content
        tool_calls = [
            p for p in assistant_msg["content"] if p.get("type") == "tool_call"
        ]
    ...
    def test_jsonl_preserves_multiple_tool_calls(self):
        ...
        tool_calls = [
            p for p in assistant_msg_data["content"] if p.get("type") == "tool_call"
        ]
...
class TestJSONAnthropicFormat:
    ...
    def test_anthropic_format_preserves_tool_use(self):
        ...
        # Find tool_use block
        tool_use_blocks = [
            b for b in assistant_msg_data["content"] if b.get("type") == "tool_use"
        ]
