...
from ctk.core.commands.organization import (OrganizationCommands,
...
class TestOrganizationCommands:
    ...
    @pytest.mark.unit
    def test_get_conversation_id_from_prefix_resolution(
        self, org_commands, mock_navigator
    ):
    ...
    @pytest.mark.unit
    def test_get_conversation_id_prefix_resolution_fails(
        self, org_commands, mock_navigator
    ):
    ...
    @pytest.mark.unit
    def test_get_conversation_id_from_current_path_not_in_conversation(
        self, org_commands
    ):
    ...
    def test_unstar_current_conversation(self, org_commands, mock_db):
        ...
        with patch.object(VFSPathParser, "parse", return_value=mock_parsed):
            ...
            assert result.success is True
            mock_db.star_conversation.assert_called_once_with(
                "conv_87654321", star=False
            )
    ...
    def test_unarchive_with_explicit_id(self, org_commands, mock_db, mock_navigator):
        ...
        mock_db.archive_conversation.assert_called_once_with(
            "conv_12345678", archive=False
        )
    ...
    @pytest.mark.unit
    def test_title_explicit_short_id_and_title(
        self, org_commands, mock_db, mock_navigator
    ):
    ...
    def test_title_database_error(self, org_commands, mock_db):
        ...
        with patch.object(VFSPathParser, "parse", return_value=mock_parsed):
            mock_db.update_conversation_metadata.side_effect = Exception(
                "Update failed"
            )
    ...
    @pytest.mark.unit
    def test_archive_handles_stdin_parameter(
        self, org_commands, mock_db, mock_navigator
    ):
    ...
    @pytest.mark.unit
    def test_multiple_operations_same_conversation(
        self, org_commands, mock_db, mock_navigator
    ):
