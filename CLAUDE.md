...
- Inline images: `MessageContent.images` (existing field) renders below the message bubble via `textual-image`'s `AutoImage` (Sixel, Kitty TGP, or Halfcell auto-detection). Source types handled: local `path`, base64 `data` (written to temp file, cleaned up at app exit), remote `url` (caption-only, no fetch on the UI thread). See `ctk/tui/images.py`.
