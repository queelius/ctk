...
- **Never name a Textual `Message` subclass with a leading underscore.** Textual derives the handler name as `"on_" + camel_to_snake(ClassName)`, and current Textual preserves the leading underscore, so `_ChatDone` dispatches to `on__chat_done` (double underscore), which never matches the defined `on_chat_done`. The message is then silently dropped: no error, the handler just never fires. This broke the entire TUI chat path (assistant replies never rendered, `_turn_active` never cleared, the input stayed read-only and the turn looked permanently stuck) and shipped silently because the chat-worker path had no test. Name app-level message classes `ChatDone` / `StreamToken` etc. (no underscore). `tests/unit/test_tui_chat_dispatch.py` has a structural guard asserting every `ctk.tui.app` `Message` subclass resolves to a real `on_*` handler; keep it. (Older Textual stripped the underscore, so this regressed on a Textual upgrade.)
